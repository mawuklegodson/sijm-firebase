...
export const DEFAULT_SETTINGS = {
  ...
  features: {
    videoSermonsEnabled: false,  // Toggle: false = "Coming Soon", true = fully active
    videoSermonsComingSoonText: 'Video sermons are coming soon! Stay tuned.',
    videoSermonsComingSoonDate: '',
  },
  liveStream: {
    // YouTube Data API v3
    youtubeApiKey:    '',   // AIza...
    ...
    return onSnapshot(resourcesQuery, (snapshot) => {
      const allResources = snapshot.docs.map(doc => {
        ...
        return {
          ...
          youtubeUrl: res.youtube_url || res.youtubeUrl || '',
          isVideoSermon: !!(res.is_video_sermon || res.isVideoSermon),
          resourceType: res.resource_type || res.resourceType || 'audio',
          date: res.date,
          author: res.author || '',
          fileSize: res.file_size || res.fileSize || '',
          downloadCount: res.download_count || res.downloadCount || 0,
          accessLevel: mappedLevel,
          isFeatured: !!(res.is_featured || res.isFeatured),
          notifyUsers: !!(res.notify_users || res.notifyUsers),
    ...
    addResource: async (res: any) => {
      ...
      try {
        await addDoc(collection(db, 'resources'), {
          ...
          youtube_url: res.youtubeUrl || '',
          youtubeUrl: res.youtubeUrl || '',
          is_video_sermon: !!res.isVideoSermon,
          isVideoSermon: !!res.isVideoSermon,
          resource_type: res.resourceType || 'audio',
          resourceType: res.resourceType || 'audio',
          date: res.date,
          author: res.author || '',
